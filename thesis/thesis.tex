...
\begin{tabular}{|l|p{\textwidth*4/5}|}
	\toprule
	Name & \textbf{Pairing-less Connection} \\ \midrule
	...
	Name & \textbf{Clock Synchronization} \\ \midrule
	...
	Name & \textbf{Database Synchronization} \\ \midrule
...
\end{table}
	...
	\label{Decentralized, Distributed Computing}
		...
		The combination of processes with the same communication partners, single-digit (bytes) payloads of a and  When a node tries to become coordinator (see figure \ref{figure:coordinator exponential backoff}) the initial state requests from nearby nodes are a good opportunity to run the clock synchronization
